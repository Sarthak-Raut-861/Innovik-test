...
export class TouchCollector extends BaseCollector {
    ...
    boundTouchStart;
    boundTouchEnd;
    constructor(privacyManager, maxSamples = 50) {
        super();
    ...
    start() {
        if (this.active || !BrowserUtils.isBrowser() || !BrowserUtils.hasTouch())
            return;
        this.active = true;
        this.paused = false;
    ...
    handleTouchStart(event) {
        if (!this.isActive() || !this.privacyManager.isTouchEnabled())
            return;
    ...
    handleTouchEnd(event) {
        if (!this.isActive() || !this.privacyManager.isTouchEnabled() || this.touchStartTime === 0)
            return;
